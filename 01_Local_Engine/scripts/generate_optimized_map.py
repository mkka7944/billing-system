#!/usr/bin/env python3
"""
Optimized field staff map generator with improved UI and performance.
"""
...
def generate_html_map(data_by_location, output_file):
    """Generate optimized HTML file with improved UI"""
...
''' + ''.join([f'''                    <div class="tehsil-group" id="tehsil-{j}">
                        ...
                        <div class="collapsible-content">
''' + ''.join([f'                            <div class="legend-item"><div class="legend-color" style="background-color:{mcuc["color"]};"></div>{mcuc["name"]} <span class="counts">({mcuc["count"]}-{mcuc["domestic"]}/{mcuc["commercial"]})</span></div>\n' for mcuc in tehsil_group["mcucs"]]) + '''                        </div>
                    </div>
...
''' for i, district_group in enumerate(location_groups)]) + '''
    ...
    <script>
        ...
        }).setView([32.0833, 72.6667], 11); // Sargodha coordinates with better zoom
        ...
        function resetFilters() {
            ...
            changeBaseLayer();
            map.setView([32.0833, 72.6667], 11); // Default to Sargodha
            markers.clearLayers();
    ...
    print(f"Generated optimized map file: {output_file}")
...
def main():
    print("==============================================")
    print("  Optimized Field Staff Map Generator")
    print("==============================================\n")
    ...
    print("  Features:")
    print("  - Horizontal filter layout for mobile")
    print("  - Smart zoom control (no excessive zooming)")
    print("  - Performance optimized (3000 marker limit)")
    print("  - Totals shown as Total-Domestic/Commercial")
    print("  - Touch-friendly interface")
    print("  - Collapsible legend with hierarchy")
    print("==============================================")
