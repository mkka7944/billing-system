#!/usr/bin/env python3
"""
Advanced Survey Extractor v3 (Name Fix)
- Fix: cleans up "Surveyor Name" to remove CNIC (e.g., "Sami Ullah/38402..." -> "Sami Ullah").
- Core: Full Field Explosion (Answers), Combined Master File, Readable Headers.
"""
...
def clean_surveyor_name(rec):
    """
    Tries to get a clean name.
    1. Checks 'added_by' (if it's a string name).
    2. Falls back to 'added_by_str' and strips the CNIC (split by / or ().
    """
    ...
    # Fallback to the composite string "Name/CNIC"
    composite = rec.get("added_by_str", "")
    if composite:
        # Split by '/' or '(' to remove CNIC
...
def flatten_record(rec, district_name, tehsil_name, uc_name):
    """
    Converts a raw nested record into a flat dictionary with business headers.
    """
    
    # 1. Base Metadata
    flat = {
        "Survey ID": rec.get("id") or rec.get("survey_id"),
        "Surveyor Name": clean_surveyor_name(rec), # <--- UPDATED HERE
...
# -----------------------------
def main():
    print("=== Advanced Survey Extractor v3 (Name Fix) ===\n")
    ...
    if mode == "1":
        ...
        if found:
            ...
            print("Warning: area_id not found in CSV. Using raw ID.")
            target_areas_list = [{"area_id": area_id_in, "area_name": f"Area_{area_id_in}", "district_name": "Unknown", "tehsil_name": "Unknown"}]
    else:
    ...
    for idx, area in enumerate(target_areas_list, 1):
        ...
        print(f"[{idx}/{len(target_areas_list)}] Processing: {u_name}...")
    ...
    # 4. Save Master File
    if MASTER_DATA:
        print("\n🔗 Generating Combined Master File...")
