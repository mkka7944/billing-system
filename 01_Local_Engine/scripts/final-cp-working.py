    ...
    <script>
        ...
        function createMarkers(filterDistrict = 'all', filterTehsil = 'all', filterMCUC = 'all', filterType = 'all') {
            ...
            if (markerCount === totalCount) {
                ...
                if (bounds.length > 0) {
                    ...
                    map.fitBounds(boundsGroup, {padding: [50, 50], maxZoom: 14});                } else {
        ...
        function showSurveyorStats() {
            ...
            currentFilteredSurveys.forEach(function(survey) {
                ...
                if (surveyDateTime) {
                    stats[name].submissions.push({dateTime: surveyDateTime, time: survey.survey_time});
                }
            });
            ...
            Object.keys(stats).forEach(function(name) {
                var stat = stats[name];
                stat.submissions.sort(function(a, b) {return a.dateTime - b.dateTime;});
                ...
                if (stat.submissions.length > 1) {
                    ...
                    for (var i = 1; i < stat.submissions.length; i++) {
                        ...
                        if (isLunchHour(prevTime) || isLunchHour(currTime)) continue;
                        var idleMs = currTime - prevTime;
                        var idleMin = Math.floor(idleMs / (1000 * 60));
                        if (idleMin > maxIdleMinutes) maxIdleMinutes = idleMin;
                    }
                    stat.idleMinutes = maxIdleMinutes;
                    
                    if (maxIdleMinutes > 120) {
                        stat.idleWarning = 'ABSENT';
                    } else if (maxIdleMinutes >= 30) {
                        stat.idleWarning = 'WARNING';
                    }
                }
                
                stat.pmCount = 0;
                stat.submissions.forEach(function(submission) {
                    if (isPMHours(submission.dateTime)) stat.pmCount++;
                });
            });
            ...
            var totalInSelection = currentFilteredSurveys.length;
            var district = document.getElementById('districtFilter').value || 'All';
            var tehsil = document.getElementById('tehsilFilter').value || 'All';

            // Clean Header
            var html = '<div style="background: #f8f9fa; color: #2c3e50; padding: 20px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #3498db;">' +
                       '<h2 style="margin: 0 0 10px 0; font-size: 22px; font-weight: 600;">Surveyor Leaderboard</h2>' +
                       '<p style="margin: 0; font-size: 14px;">District: <strong>' + district + '</strong> | Tehsil: <strong>' + tehsil + '</strong> | Total Submissions: <strong>' + totalInSelection + '</strong></p>' +
                       '</div>';
            ...
            sortedNames.forEach(function(name, index) {
                ...
                if (s.idleWarning === 'WARNING') statusColor = '#f39c12';
            ...
            html += '<div style="margin-top: 20px; padding: 15px; background: #f8f9fa; border-radius: 8px; border-left: 4px solid #3498db;">' +
                    '<strong style="color: #2c3e50; font-size: 14px;">Total Surveyors: ' + sortedNames.length + ' | Total Submissions: ' + totalInSelection + '</strong>' +
                    '</div>';
