            ...
            for mcuc in sorted_mcucs:
                ...
                for record in records:
                    ...
                    survey_points.append({
                        ...
                        'surveyor_name': str(record.get('Surveyor Name', '')).strip(),  # Add surveyor name
...
''' for i, district_group in enumerate(location_groups)]) + '''
        ...
        <div class="credits-container">
            ...
            <div class="credits">Map created by Kashif Khalil</div>
        </div>
    </div>
    ...
    <script>
        ...
        // Add zoom level update listener
        ...
        // Create a regular layer group instead of marker cluster group
        ...
        function createMarkers(filterDistrict = 'all', filterTehsil = 'all', filterMCUC = 'all', filterType = 'all') {
            ...
            surveyData.some(function(survey) {
                ...
                // Create custom icon with circle styling (smaller circles)
...
def process_csv_files():
    ...
    for csv_file in csv_files:
        ...
        try:
            ...
            # Check for survey data columns (location data)
            survey_columns = ['Latitude', 'Longitude', 'District', 'Tehsil', 'Union Council']
            # Check for billing history columns (may have location data)
            billing_columns = ['District', 'Tehsil', 'UC']
            
            # Check if this is a survey data file
            if all(col in df.columns for col in survey_columns):
                # Survey data file - use as is
                location_source = 'survey'
                lat_col, lon_col, district_col, tehsil_col, uc_col = 'Latitude', 'Longitude', 'District', 'Tehsil', 'Union Council'
            # Check if this is a billing history file
            elif all(col in df.columns for col in billing_columns) and 'Latitude' in df.columns and 'Longitude' in df.columns:
                # Billing history file - use appropriate columns
                location_source = 'billing'
                lat_col, lon_col, district_col, tehsil_col, uc_col = 'Latitude', 'Longitude', 'District', 'Tehsil', 'UC'
            else:
                print(f"Skipping {csv_file} - missing required location columns")
                continue
            
            print(f"Processing {csv_file} as {location_source} data...")
            
            # Process each row
            for _, row in df.iterrows():
                # Skip if location is missing
                lat = str(row.get(lat_col, '')).strip()
                lon = str(row.get(lon_col, '')).strip()
                ...
                tehsil = str(row.get(tehsil_col, '')).strip()
                uc = str(row.get(uc_col, '')).strip()
