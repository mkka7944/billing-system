#!/usr/bin/env python3
"""
Simple script to generate a self-contained HTML map file for field staff
that works offline on Android phones.
"""

import pandas as pd
import os
import json
from collections import defaultdict

# Define input folder
INPUT_FOLDER = os.path.join("..", "outputs", "scraped_data_appsheet")

def get_marker_color(mc_uc):
    """Generate a consistent color for each MC/UC"""
    # Simple hash-based color generation for consistency
    hash_val = hash(mc_uc) % 360
    return f"hsl({hash_val}, 70%, 45%)"

def generate_html_map(data_by_mc_uc, output_file):
    """Generate self-contained HTML file with interactive map"""
    
    # Prepare survey data as JSON
    survey_points = []
    mcuc_list = []
    
    for mcuc, records in data_by_mc_uc.items():
        color = get_marker_color(mcuc)
        mcuc_list.append({"name": mcuc, "color": color})
        for record in records:
            survey_points.append({
                'survey_id': str(record.get('Survey ID', '')),
                'mc_uc': mcuc,
                'district': str(record.get('District', '')),
                'tehsil': str(record.get('Tehsil', '')),
                'consumer_type': str(record.get('Consumer Type', '')),
                'house_type': str(record.get('House Type', '')),
                'survey_date': str(record.get('Survey Date', '')),
                'location': str(record.get('Location', '')),
                'color': color
            })
    
    survey_data_json = json.dumps(survey_points)
    mcuc_data_json = json.dumps(mcuc_list)
    
    html_content = '''<!DOCTYPE html>
<html>
<head>
    <title>Field Survey Map - MC/UC View</title>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <style>
        body {{ margin: 0; padding: 0; font-family: Arial, sans-serif; }}
        #map {{ position: absolute; top: 0; bottom: 0; width: 100%; }}
        .info {{ 
            padding: 6px 8px; 
            font: 14px/16px Arial, Helvetica, sans-serif; 
            background: white; 
            background: rgba(255,255,255,0.8); 
            box-shadow: 0 0 15px rgba(0,0,0,0.2); 
            border-radius: 5px; 
            position: fixed;
            top: 10px;
            right: 10px;
            z-index: 1000;
            max-width: 300px;
        }}
        .legend {
            background: white;
            padding: 10px;
            border-radius: 5px;
            box-shadow: 0 0 15px rgba(0,0,0,0.2);
            max-height: 300px;
            overflow-y: auto;
        }
        .legend-item {
            margin: 2px 0;
            display: flex;
            align-items: center;
        }
        .legend-color {
            width: 15px;
            height: 15px;
            margin-right: 5px;
            border: 1px solid #666;
        }
        .controls {
            background: white;
            padding: 10px;
            border-radius: 5px;
            box-shadow: 0 0 15px rgba(0,0,0,0.2);
            margin-bottom: 10px;
        }
        select, button {
            padding: 5px;
            margin: 2px;
        }
    </style>
</head>
<body>
    <div id="map"></div>
    
    <div class="info">
        <div class="controls">
            <select id="mcucFilter">
                <option value="all">Show All MC/UC</option>
''' + ''.join([f'                <option value="{item["name"]}">{item["name"]}</option>\n' for item in mcuc_list]) + '''
            </select>
            <button onclick="filterMarkers()">Filter</button>
            <button onclick="resetView()">Reset View</button>
        </div>
        <div class="legend">
            <h4>MC/UC Legend</h4>
''' + ''.join([f'            <div class="legend-item"><div class="legend-color" style="background-color:{item["color"]};"></div>{item["name"]}</div>\n' for item in mcuc_list]) + '''
        </div>
    </div>

    <!-- Leaflet CSS -->
    <link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css" />
    
    <!-- Leaflet JS -->
    <script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script>
    
    <script>
        // Initialize the map
        var map = L.map('map').setView([32.0, 73.0], 8);
        
        // Add base tile layer (OpenStreetMap)
        L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
            attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
        }).addTo(map);
        
        // Survey data
        var surveyData = ''' + survey_data_json + ''';
        
        // Create marker layers
        var markers = L.layerGroup().addTo(map);
        
        // Function to create markers
        function createMarkers(filterMCUC = 'all') {
            // Clear existing markers
            markers.clearLayers();
            
            // Process each survey point
            surveyData.forEach(function(survey) {
                // Skip if filtering and MC/UC doesn't match
                if (filterMCUC !== 'all' && survey.mc_uc !== filterMCUC) {
                    return;
                }
                
                // Parse location
                var coords = survey.location.split(',');
                if (coords.length !== 2) return;
                
                var lat = parseFloat(coords[0]);
                var lng = parseFloat(coords[1]);
                
                if (isNaN(lat) || isNaN(lng)) return;
                
                // Create popup content
                var popupContent = 
                    '<b>Survey ID:</b> ' + survey.survey_id + '<br>' +
                    '<b>MC/UC:</b> ' + survey.mc_uc + '<br>' +
                    '<b>District:</b> ' + survey.district + '<br>' +
                    '<b>Tehsil:</b> ' + survey.tehsil + '<br>' +
                    '<b>Consumer Type:</b> ' + survey.consumer_type + '<br>' +
                    '<b>House Type:</b> ' + survey.house_type + '<br>' +
                    '<b>Date:</b> ' + survey.survey_date;
                
                // Get color for this MC/UC
                var color = survey.color || '#3388ff';
                
                // Create circle marker
                var marker = L.circleMarker([lat, lng], {
                    radius: 6,
                    fillColor: color,
                    color: "#000",
                    weight: 1,
                    opacity: 1,
                    fillOpacity: 0.8
                }).bindPopup(popupContent);
                
                // Add to map
                marker.addTo(markers);
            });
        }
        
        // Filter markers function
        function filterMarkers() {
            var select = document.getElementById('mcucFilter');
            var selectedValue = select.value;
            createMarkers(selectedValue);
        }
        
        // Reset view function
        function resetView() {
            map.setView([32.0, 73.0], 8);
            document.getElementById('mcucFilter').value = 'all';
            createMarkers('all');
        }
        
        // Initialize markers
        createMarkers();
    </script>
</body>
</html>'''
    
    # Write to file
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(html_content)
    
    print(f"Generated map file: {output_file}")

def process_csv_files():
    """Process CSV files and generate map data"""
    csv_files = [f for f in os.listdir(INPUT_FOLDER) if f.endswith('.csv') and 'MASTER' not in f]
    
    if not csv_files:
        print("No CSV files found in the input folder")
        return
    
    # Collect data by MC/UC
    data_by_mc_uc = defaultdict(list)
    
    for csv_file in csv_files:
        file_path = os.path.join(INPUT_FOLDER, csv_file)
        print(f"Processing {csv_file}...")
        
        try:
            # Read CSV file
            df = pd.read_csv(file_path, encoding='utf-8-sig', low_memory=False)
            
            # Required columns check
            required_columns = ['Location', 'District', 'Tehsil', 'Union Council']
            if not all(col in df.columns for col in required_columns):
                print(f"Skipping {csv_file} - missing required columns")
                continue
            
            # Process each row
            for _, row in df.iterrows():
                # Skip if location is missing
                if not row.get('Location') or ',' not in str(row['Location']):
                    continue
                
                # Create MC/UC identifier
                district = str(row.get('District', '')).strip()
                uc = str(row.get('Union Council', '')).strip()
                mc_uc = f"{district} - {uc}"
                
                # Add to collection
                data_by_mc_uc[mc_uc].append(dict(row))
                
        except Exception as e:
            print(f"Error processing {csv_file}: {e}")
    
    # Generate HTML map
    output_file = os.path.join("..", "outputs", "field_staff_map.html")
    generate_html_map(data_by_mc_uc, output_file)
    
    # Print summary
    total_records = sum(len(records) for records in data_by_mc_uc.values())
    print(f"\nSummary:")
    print(f"- Processed {len(csv_files)} CSV files")
    print(f"- Total survey records: {total_records}")
    print(f"- Unique MC/UC areas: {len(data_by_mc_uc)}")
    print(f"- Map file generated: {output_file}")

def main():
    print("==============================================")
    print("  Field Staff Map Generator")
    print("==============================================\n")
    
    process_csv_files()
    
    print("\n==============================================")
    print("  Map Generation Complete!")
    print("  The HTML file can be opened directly on Android phones")
    print("  No internet connection required after download")
    print("==============================================")

if __name__ == "__main__":
    main()